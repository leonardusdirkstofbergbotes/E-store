
<div id="footy_header">
    <a href="#"><img src="./img/icons/facebook.png"> Nate's Clothing</a>
    <a href="#"><img src="./img/icons/twitter.png"> #Nate'sClothing</a>
    <a href="#"><img src="./img/icons/email.png"> natesclothing@gmail.com</a>
</div>

<div id="footy">
    <div id="left">
        <div id="leftA">
            <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3294.314033748079!2d19.007320314346202!3d-34.34248498052796!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x1dcdd9dccaf23b01%3A0xd99af027e1e23165!2s21%20Neethling%20St%2C%20Kleinmond%2C%207195!5e0!3m2!1sen!2sza!4v1588096270841!5m2!1sen!2sza" 
            width="250" height="250" id="maps" frameborder="0" style="border:0;" allowfullscreen="" aria-hidden="false" tabindex="0"></iframe>   
            <img src="./img/hero/store_front.jpg">
        </div>
    <div id="stack">
        <h3>Visit our branch</h3>
        <p>Cape Town, South Africa</p>
        <p>21 Neethling st</p>
        <p>7430</p>
        <p></p>
        <p></p>
        <h3>Trading hours</h3>
        <pre>Mon - Fri:     08:00 - 16:00</pre>
        <pre>Sat:           09:00 - 13:00</pre>
        <pre>Sun:           Closed</pre>
    </div>
    
    </div>
    <div id="right">
    <form method="post" class="wrapper" id="contact_us" enctype="multipart/form-data">

        <h1> Get in touch </h1>
    
        <div class="input"> 
            <label for='user_name'>Your name </label>
            <input type="text" class="input" maxlength="255" minlength="3" name="user_name" placeholder="example Joe" title="">
        </div>

        <div class="input">  
            <label for='user_email'>Email adress </label>
            <input type="email" class="input" name="user_email" maxlength="128" placeholder="example@gmail.com" title="">
        </div>

        <div class="input">  
            <label for='message'>Your message </label>
            <textarea class="input" name="message" rows="4" title=""></textarea>
        </div>

        <div id="send_message">
            <button class="light_pri_button">Send message</button>
        </div>
        
    </form>
    </div>
</div> 

<div id="footer_bottom">
    <div id="payment_icons">
        <img src="./img/icons/visa.png">
        <img src="./img/icons/paypal.png">
        <img src="./img/icons/american-express.png">
    </div>
    <div id="nate_bottom">
        <p>Nate's clothing store 2020 </p>
    </div>
</div>